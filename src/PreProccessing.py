...
def main():
    ...
    cols_to_stack = ["MaxTemp", "MinTemp"]
    cols_not_to_stack = [col for col in data.columns if col not in cols_to_stack]

    temp_stacked = (
        data
        .drop(columns=["Feed_ID", "Quantity"])
        .set_index(cols_not_to_stack)
        .stack()
        .reset_index()
        .rename(columns={"level_9" : "Feature", 0 : "Value"})
    )
    temp_stacked = data[["MaxTemp", "MinTemp"]].stack()
